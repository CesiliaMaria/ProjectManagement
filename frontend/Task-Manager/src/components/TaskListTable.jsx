...
const TaskListTable = ({ tableData }) => {
  const getStatusBadgeColor = (status) => {
    switch (status) {
      ...
      default:
        return "bg-gray00 text-gray-500 border border-gray-200";
    }
  };
      ...
      <table className="min-w-full">
        <thead>
          <tr className="text-left">
            ...
            <th className="py-3 px-4 text-gray-800 font-medium text-[13px] hidden md:table-col">
              Created On
            </th>
          </tr>
        </thead>
        <tbody>
          {tableData.map((task) => {
            <tr key={task._id} className="border-t border-gray-200">
              <td className="my-3 mx-4 text-gray-700 text-[13px] lie-clamp-1 overflow-hidden">
                {task.title}
              </td>
